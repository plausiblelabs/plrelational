...
private func graphvizDump(nodes: [QueryPlanner.Node], showChildPointers: Bool = false, auxNodeInfo: (Int) -> String? = { _ in nil }, printer print: (String) -> Void = { print($0) }) {
    ...
    for (index, node) in nodes.enumerated() {
        ...
        let label = [nodename(index), node.debugName.map({ "NAME: \($0)" }), node.approximateCount.map({ "~\($0) rows" }), String(describing: node.scheme.attributes), opString].flatMap({ $0 }).joined(separator: " ")

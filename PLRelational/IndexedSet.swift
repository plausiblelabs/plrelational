...
public struct IndexedSet<Element: IndexableValue> {
    ...
    fileprivate var index: [Element.Index: MutableBox<[Element.IValue: MutableBox<Set<Element>>]>]
...
extension IndexedSet {
    ...
    public func values(matchingKey: Element.Index, value: Element.IValue) -> Set<Element> {
...
extension IndexedSet {
    fileprivate mutating func add(indexedValue: Element.IValue, element: Element, toDictionary: inout [Element.IValue: MutableBox<Set<Element>>]) {
    ...
    fileprivate mutating func remove(indexedValue: Element.IValue, element: Element, fromDictionary: inout [Element.IValue: MutableBox<Set<Element>>]) {
...
public protocol IndexableValue: Hashable {
    ...
    // Note: This has the 'I' prefix to avoid colliding with other associated types (e.g. Row wants to conform to both
    ...
    associatedtype IValue: Hashable
    
    /// Return a value for a given index.
    func value(index: Index) -> IValue
}
